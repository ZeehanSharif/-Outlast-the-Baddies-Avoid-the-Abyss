...
class Monster: public BoardCell {
    ...
    public:
    	...
    	virtual void attemptMoveTo(size_t& newR, size_t& newC, size_t hRow, size_t hCol) {
            ...
            //                direction(s)
            //------------------------------------------------------------------------
            size_t currentR = getRow();
            size_t currentC = getCol();

            int vertStep = (hRow > currentR) ? 1 : (hRow < currentR) ? -1 : 0;
            int horzStep = (hCol > currentC) ? 1 : (hCol < currentC) ? -1 : 0;
            ...
            newR = (vertStep != 0) ? currentR + vertStep : currentR;
            newC = (horzStep != 0) ? currentC + horzStep : currentC;
        }
